...
pub fn vmx_test() {
    ...
    unsafe {
        ...
        let mut vmxon_region = vec![0u8; 4096];
        let mut vmxon_ptr_page = vec![0u8; 4096];
        let mut vmcs_region = vec![0u8; 4096];
        let mut vmcs_ptr_page = vec![0u8; 4096];
        let mut ept_root = vec![0u8; 4096];
        let mut pdpt = vec![0u8; 4096];
    
        // Get access to the page table
        ...
        let vmxon_region_paddr = page_table.translate(
            &mut pmem, VirtAddr(vmxon_region.as_mut_ptr() as u64))
            .unwrap().page.unwrap().0;

        // Create a 4-KiB zeroed out physical page to point to the vmxon page
        let vmxon_ptr_page_paddr = page_table.translate(
            &mut pmem, VirtAddr(vmxon_ptr_page.as_mut_ptr() as u64))
            .unwrap().page.unwrap().0;

        vmxon_ptr_page[..8].copy_from_slice(
            &(vmxon_region_paddr.0 as usize).to_le_bytes());

        print!("vmxon region allocated at       {:#x}\n\
                vmxon pointer page allocated at {:#x}\n",
            vmxon_region_paddr.0,
            vmxon_ptr_page_paddr.0);
        ...
        // number
        let vmcs_region_paddr = page_table.translate(
            &mut pmem, VirtAddr(vmcs_region.as_mut_ptr() as u64))
            .unwrap().page.unwrap().0;
        vmcs_region[..4].copy_from_slice(&vmcs_revision_number.to_le_bytes());

        // Create a 4-KiB zeroed out physical page to point to the vmxon page
        let vmcs_ptr_page_paddr = page_table.translate(
            &mut pmem, VirtAddr(vmcs_ptr_page.as_mut_ptr() as u64))
            .unwrap().page.unwrap().0;
        vmcs_ptr_page[..8].copy_from_slice(
            &(vmcs_region_paddr.0 as usize).to_le_bytes());
        ...
        let ept_root_paddr = page_table.translate(
            &mut pmem, VirtAddr(ept_root.as_mut_ptr() as u64))
            .unwrap().page.unwrap().0;

        let pdpt_paddr = page_table.translate(
            &mut pmem, VirtAddr(pdpt.as_mut_ptr() as u64))
            .unwrap().page.unwrap().0;

        let pml4e_entry = (pdpt_paddr.0 as usize) | 7;
        ept_root[..8].copy_from_slice(&pml4e_entry.to_le_bytes());
        pdpt[..8].copy_from_slice(&((1 << 7) | 7usize).to_le_bytes());

        cpu::vmwrite(EPT_POINTER, ept_root_paddr.0 as u64 | (3 << 3));
